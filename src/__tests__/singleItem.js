...
describe("<SingleItem />", () => {
  ...
  it("Testing for images - componentDidMount", done => {
    ...
    setTimeout(() => {
      ...
      expect(wrapper.find(".hvrbox-text").text()).toBe(
        "Mars in ColorMars in ColorCreated: 15 years ago"
      );
  ...
  it("If no data is returned error message is given", done => {
    ...
    setTimeout(() => {
      ...
      // HTML PAGE ELEMENTS
      expect(wrapper.find(".ErrorLoad").text()).toBe(
        "Request failed, please try another search"
      );
      done();
    }, 50);
  });
